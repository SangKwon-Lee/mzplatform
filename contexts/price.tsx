...
import SocketManager from '../lib/price-manager/socket-manager';
import { usePageVisibility } from 'react-page-visibility';
import { useAppDispatch } from '../lib/redux/hooks';
import { set, unset } from '../lib/redux/stocks';
import { IndexValue } from '../commons/types';

const PRICE_HOST = process.env.NEXT_PUBLIC_PRICE_HOST;
const indices = [
  {
    symbol: 'kospi',
    code: '001',
...
export function PriceProvider({ children }: Props) {
  ...
  const registRealtimePrice = async (stockcodes: Array<any>, push = true) => {
    ...
    stockcodes.map((stockcode) => {
      // 이미 존재하면 추가하지 않는다.
      if (!queryManager.current || listeningStocks.current.find((item) => item === stockcode)) return;
      if (push) {
      ...
      manager.registerReal('KBRSKXS3', 'rl_tm_key', ['A' + stockcode], 'app', function (queryData: any) {
        if (!queryData) {
          return;
        }
        realtimeDataListener(stockcode, queryData.getBlockData('OutBlock1')[0]);
      });
    });
    console.log('[SocketContext] listening stocks: ', listeningStocks.current);
  };
  ...
  const realtimeDataListener = (stockcode: string, data: any) => {
    dispatch(
      set({
        ...
        diff: parseFloat(data.up_dwn_r_p2) > 0 ? parseInt(data.bdy_cmpr) : -parseInt(data.bdy_cmpr),
        high: parseInt(data.hgh_prc),
        low: parseInt(data.lw_prc),
        last: parseFloat(data.up_dwn_r_p2) > 0 ? parseInt(data.now_prc) - parseInt(data.bdy_cmpr) : parseInt(data.now_prc) + parseInt(data.bdy_cmpr),
      }),
    );
  };
        ...
        manager.sendProcessByName(
          ...
          function (queryData: any) {
            ...
            const data: IndexValue[] = indices.map((item, i) => {
              ...
              const before_price = price.pricejisu + (price.sign >= '4' ? price.change : -price.change);
              ...
              const theIndex: IndexValue = {
                ...
                diff: diff,
                ratio: parseFloat(((diff / before_price) * 100).toFixed(2)),
              };
              return theIndex;
            });
            resolve(data);
