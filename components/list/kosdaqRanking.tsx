...
import { fetchPriceKosdaqRanking } from '../../lib/redux/price';
...
export default function KospiRankingViewList() {
  ...
  const { priceListKosdaq } = useAppSelector((state) => state.price);
  ...
  useEffect(() => {
    if (Array.isArray(priceListKosdaq) && priceListKosdaq.length > 0) {
      const fetchedStocks: string[] = [];
      priceListKosdaq.map((data) => {
  ...
  useEffect(() => {
    //@ts-ignore
    dispatch(fetchPriceKosdaqRanking());
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);
      ...
      {isInViewport && (
        <FadeIn delay={150} transitionDuration={1000}>
          <MostViewContentsWrapper>
            {Array.isArray(priceListKosdaq) && priceListKosdaq.length > 0 ? (
              priceListKosdaq.map((data, index) => (
...
const MostViewContentsBox = styled.div`
